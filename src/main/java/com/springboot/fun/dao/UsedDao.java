...
public interface UsedDao {
     ...
     * */
    //获取二手列表
    List<Used> finUsedList(Map<String, Object> usedMap);
    ...
    //用户点赞
    Integer insertUsedLike(UsedLike usedLike);
    ...
    //获取用户发布所有评论
    List<UsedComment> findUsedAllComment(String id);

    //给用户评论
    Integer insertUsedComment(UsedComment usedComment);
    ...
    //个人中心 我的发布
    List<Used> findAllSuccessUserList(Map<String, Object> usedMap);
    ...
    //获取全部的二手（不分学校）
    List<Used> finAllUsedList(Map<String,Object> usedMap);
    //获取发布和评论的返回内容
    List<FunUsedState> findUsedState(int type);
    /*后台*/
    //二手筛选列表
    List<Used> searchUsedList(Map<String,Object> usedMap);
    //给列表刷新的浏览量+1
    ...
    //点击展示未读内容的消息列表
    List<UsedCommentReplyMessage> selectCommentReplyList(String userId);
    //把评论消息改为已读
