...
@Mapper
public interface AppletLoginDao {
    ...
    //添加到好友列表
    void insertShareMember(YlqFriend ylqFriend);
    ...
    //创建用户钱包
    void insertWallet(YlqWallet ylqWallet1);
    //查找用户的动力币明细
    List<YlqWalletDetail> findUserWalletDetail(String userId);
    /*查看前100名本周的排行榜*/
    List<YlqUserStep> selectWalletRankingList(Map<Object,Object> map);
    //查找用户id有没有重复，有，就再次生成
    ...
    //查看微信用户列表
    List<WxUser> findWXUserList(Map<String,Object> usedMap);
    //模糊搜索用户（昵称，主键id动力币余额，手机号，唯一id）
    List<WxUser> searchMapWXUserList(Map<String,Object> searchMap);
    //修改动力币
